  ...
  return (
    ...
    <div className={'w-full max-w-sm p-10 bg-slategray/80 rounded-2xl flex flex-col items-start shadow-2xl cursor-pointer hover:bg-slategray transition-all'}>
      <div className={'flex justify-center items-center w-full'}>
        ...
        <img alt={blog.title + ' thumbnail'} src={blog.coverImg} className='h-[200px]' width="100%" height={"200"} objectFit='cover'/>
      </div>
      ...
      <div className={styles["info-container"]}>
        <div className={styles["smallinfo"]}>
          ...
          <div style={{ width: "10px"}}></div>
          <p className={styles["small-publish-date"]}> { new Date(blog.publishedAt).toUTCString() } </p>
        </div>
      ...
      <div className={styles["blog-author-container"]}>
        <div className={styles["author-profile"]}>
          <Image src={blog.authorImg} alt={blog.authorName + " profile picture"} layout="responsive" width={"40px"} height={"40px"} objectFit="cover"/>
        </div>
...
function FirebaseBlogCard({ blog }) {
